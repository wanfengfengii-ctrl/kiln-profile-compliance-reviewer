...
裁决的 JSON。裁决完成后还可在结果区为每段填写最低、最高允许热暴露量，
点击“复核热暴露”确认烧成强度是否落在窗口内（与放行/返烧结论相互独立）。
...
- 时间字段为非负整数秒：JSON 整数或十进制整数字符串均可（字符串形式可
  无损承载超出 IEEE 754 安全整数范围的秒数）；**响应中时间字段一律以
  十进制字符串返回**，任意大整数都不丢精度。
- 温度/速率字段为十进制数（字符串或 JSON 数值均可，推荐字符串以保持精度）。
- 速率判定用有理数精确比较（温差×60 > 限制×秒差），实测值贴近限制时
  不会因舍入误判；违规项中的实测值以 50 位有效数字十进制展示。
- 成功 `200`：`{"conclusion": "放行"|"返烧", "violations": [...], "stages": [...],
  "samples": [...]}`（回传裁决所用数据，供折线与下载使用）。
- 输入错误 `422`：`{"detail": [...]}`，不产生裁决结果。
- 健康检查：`GET /api/health`。

`POST /api/exposure`（热暴露复核，与放行/返烧结论相互独立）

```json
{
  "stages": ["同 /api/adjudicate"],
  "samples": ["同 /api/adjudicate"],
  "exposure": [{"min_exposure": "7500", "max_exposure": "7500"}]
}
```

- 以该次裁决回传的阶段和采样构造分段线性温度曲线；跨阶段线段在边界
  插值切开，曲线穿越该阶段最低温度时再次切分，仅对高于最低温度的面积
  按梯形积分累加（十进制转有理数精确运算），并换算为 **°C·min**。
- `exposure` 为逐段窗口，数量必须与阶段数一致；上下限为有限非负十进制数
  且下限不高于上限。多段窗口错误按阶段输入位置稳定返回全部明细
  （`422`，`detail` 的 `loc` 含 `exposure` 下标），页面清除旧暴露报告并
  定位（高亮）相应阶段。
- 成功 `200`：`{"results": [...]}` 按阶段顺序返回，每项含
  `stage_index`、`exposure`（“分子/分母”最简分数）、`exposure_display`
  （四舍五入六位展示值）、回传的窗口与 `status`
  （`不足` / `合格` / `过量`）；先以精确值比较窗口，等于上下限判为合格。
